...
GMainWindow::GMainWindow(Core::System& system_)
    ...
    for (int i = 1; i < args.size(); ++i) {
        ...
        if (args[i] == QStringLiteral("--version") || args[i] == QStringLiteral("-v")) {
            const std::string version_string =
                std::string("LimoMada3DS ") + Common::g_scm_branch + " " + Common::g_scm_desc;
    ...
    LOG_INFO(Frontend, "LimoMada3DS Version: {} | {}-{}", Common::g_build_fullname,
...
void GMainWindow::InitializeHotkeys() {
    ...
    link_action_shortcut(ui->action_Exit, QStringLiteral("Exit LimoMada3DS"));
...
void GMainWindow::CheckForMigration() {
    ...
    if (fs::is_directory(FileUtil::GetUserPath(FileUtil::UserPath::LegacyUserDir)) &&
        ...
        if (QMessageBox::information(
                this, tr("Migration"),
                tr("LimoMada3DS has moved to a new data directory.\n\n"
...
void GMainWindow::MigrateUserData() {
    ...
    QMessageBox::information(
        this, tr("Migration"),
        tr("Data was migrated successfully. LimoMada3DS will now start.\n\n"
...
static std::optional<QDBusObjectPath> HoldWakeLockLinux(u32 window_id = 0) {
    ...
    options.insert(QString::fromLatin1("reason"),
                   QCoreApplication::translate("GMainWindow", "LimoMada3DS is running a game"));
    ...
    if (result != Core::System::ResultStatus::Success) {
        ...
        case Core::System::ResultStatus::ErrorLoader_ErrorGbaTitle:
            QMessageBox::critical(this, tr("Unsupported ROM"),
                                  tr("GBA Virtual Console ROMs are not supported by LimoMada3DS."));
            break;
...
void GMainWindow::BootGame(const QString& filename) {
    ...
    LOG_INFO(Frontend, "LimoMada3DS starting...");
    if (!is_artic) {
...
void GMainWindow::OnGameListCreateShortcut(u64 program_id, const std::string& game_path,
    ...
    const std::string comment = fmt::format("Start {:s} with the LimoMada3DS Emulator", game_title);
    ...
    case Service::AM::InstallStatus::ErrorEncrypted:
        QMessageBox::critical(this, tr("Encrypted File"),
                              tr("%1 must be decrypted "
                                 "before being used with LimoMada3DS. A real 3DS is required.")
                                  .arg(filename));
        break;
...
void GMainWindow::UninstallTitles(
    ...
    if (failed) {
        QMessageBox::critical(this, tr("LimoMada3DS"),
                              tr("Failed to uninstall '%1'.").arg(failed_name));
    } else if (!future_watcher.isCanceled()) {
        QMessageBox::information(this, tr("LimoMada3DS"),
...
void GMainWindow::OnOpenFFmpeg() {
    ...
    if (success.load()) {
        QMessageBox::information(this, tr("LimoMada3DS"), tr("FFmpeg has been sucessfully installed."));
    } else {
        QMessageBox::critical(this, tr("LimoMada3DS"),
...
bool GMainWindow::ConfirmClose() {
    ...
    QMessageBox::StandardButton answer =
        QMessageBox::question(this, tr("LimoMada3DS"), tr("Would you like to exit now?"),
...
bool GMainWindow::ConfirmChangeGame() {
    ...
    auto answer = QMessageBox::question(
        this, tr("LimoMada3DS"), tr("The game is still running. Would you like to stop emulation?"),
        QMessageBox::Yes | QMessageBox::No, QMessageBox::No);
    return answer != QMessageBox::No;
}
...
void GMainWindow::UpdateWindowTitle() {
    ...
    if (game_title_long.isEmpty()) {
        setWindowTitle(QStringLiteral("LimoMada3DS %1").arg(full_name));
    } else {
        setWindowTitle(QStringLiteral("LimoMada3DS %1 | %2").arg(full_name, game_title_long));
        render_window->setWindowTitle(QStringLiteral("LimoMada3DS %1 | %2 | %3")
                                          .arg(full_name, game_title_long, tr("Primary Window")));
        secondary_window->setWindowTitle(
            QStringLiteral("LimoMada3DS %1 | %2 | %3")
...
void LaunchQtFrontend(int argc, char* argv[]) {
    ...
    QCoreApplication::setApplicationName(QStringLiteral("LimoMada3DS"));
