...
$stateDir = Join-Path $env:LOCALAPPDATA 'MadaLime-EasyLaunch'
...
function Find-LimeExecutable {
    $candidates = @(
        (Join-Path $repo 'build\windows-mingw-relwithdebinfo\bundle\madalime.exe'),
        (Join-Path $repo 'build\windows-mingw-relwithdebinfo\bin\RelWithDebInfo\madalime.exe'),
        (Join-Path $repo 'build\windows-mingw-relwithdebinfo\bin\madalime.exe')
    )
    $found = $candidates | Where-Object { Test-Path $_ } | Select-Object -First 1
    if (-not $found) {
        $found = Get-ChildItem (Join-Path $repo 'build') -Filter 'madalime.exe' -File -Recurse -ErrorAction SilentlyContinue |
...
if (-not $exe) { throw "No se encontro madalime.exe. Compila con: .\scripts\Build-Lime3DS.ps1 -Bundle" }
...
catch { throw "No se pudo iniciar MadaLime: $($_.Exception.Message)" }
